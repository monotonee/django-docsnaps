...
snapshot, if any, and, if a change is detected, saves a new snapshot. Over time,
a the series of changes to a document are recorded. The management command can
be run from a cron job, systemd timer, or any other task scheduler.
